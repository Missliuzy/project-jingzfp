...
(function(window, $) {
  ...
  function getData() {
    ...
    $.ajax = function(opt) {
      if (
        opt.type == 'post' ||
        (opt.type == 'put' && $('.save_pop_sure').length == 0)
      ) {
        $('body').append(
          '<div class="save_pop_sure">' +
      ...
      if (typeof opt.data === 'object') {
        if (
          /delete/gim.test(opt.type) ||
          /get/gim.test(opt.type) ||
          opt.type == '' ||
          opt.type == undefined
        ) {
  ...
  function page() {
    $.fn.pageTool = function(obj) {
      ...
      function initList() {
        return getData()
          ...
          .done(function(data, textStatus, jqXHR) {
            ...
            changeEle.html(renderHtml);
            // alert(renderHtml);
            changeEle[0].innerHTML = renderHtml;
            initPageItem(data);
      ...
      function initPageItem(data) {
        ...
        var pageStr =
          '<div class="pageTool">' +
          '<div class="pageTool_left">共' +
          data.rowsCount +
          '条信息</div>' +
          '<div class="pageTool_right">' +
          '<span class="pageTool_previous ' +
          prevDis +
          '"><上一页</span>' +
          '<div class="wrap_pageItem">' +
          // list page item
          '</div>' +
          '<span class="pageTool_next ' +
          nextDis +
          '">下一页></span>' +
        ...
        if (len <= 6) {
          ...
          if (currentPage > len - 4) {
            ...
            if (currentPage === len - 3) {
              wrapItem.append(
                '<span class="pageTool_currentPage">' + currentPage + '</span>'
              );
              ...
              for (var k = 3; k >= 0; k--) {
                ...
                wrapItem.append(
                  '<span ' + currStr + '>' + (len - k) + '</span>'
                );
              }
            }
          } else {
            wrapItem.append(
              '<span>1</span><span>...</span><span class="pageTool_currentPage">' +
                currentPage +
                '</span>'
            );
      ...
      function enevtBind() {
        ...
        $('.pageTool .pageTool_next', _this).bind(
          'click',
          '.pageTool .pageTool_next',
          nextPage
        );
